...
| Timestamps | SQL `NOW()` was parsed eight hours ahead of JavaScript time when the database server used Asia/Singapore. | Initialize database sessions in UTC; keep daily counts, report hour/week groupings, date filters and sticker expiry aligned with Philippine time. |
...
- 19 authenticated administrator/guard pages loaded without uncaught JavaScript errors while external page resources were blocked. Reports rendered using the local chart bundle, grouped a dedicated gate fixture by its Philippine hour, and displayed their fallback when that bundle was deliberately blocked.
